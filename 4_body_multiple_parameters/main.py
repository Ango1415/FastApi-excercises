...
@app.put("/items/{item_id}/body_and_query/")
async def update_item_body_and_query(
    *,
    item_id: int,
    item: Item,
    user: User,
    ...
    * (Positional-only Arguments Separator): When used '*' alone, before other named parameters, it signifies that all
    parameters following it must be passed as keyword arguments, not positional arguments [2]. This helps enforce clarity
    and prevent errors in argument passing.
    Example: def my_function(a, b, *, c, d): ---> my_function(1, 2, c=3, d=4)  # Works
    """
